...
| role | Подпись рядом с ФИО в меню | обязательное | `owner` = Владелец, `user` = Пользователь; отображение и разграничение прав (шаги 3–4) |
...
| assigned_employee_id | Select (employees с role=user) | опционально | закрепление объекта за пользователем; только владелец задаёт; пользователь видит только объекты, где assigned_employee_id = его id |
